...
- (BOOL)send
{
	...
	NSString *source = [item customValueForKey:@"source"] ? [item customValueForKey:@"source"] : @"";
	...
	if (item.shareType == SHKShareTypeURL) {
        ...
        NSString* link = [item.URL absoluteString];
        NSString* caption = [item customValueForKey:@"caption"]?[item customValueForKey:@"caption"]:@"";
        
        if ([link length] != 0 && [caption length] != 0 && [item customBoolForSwitchKey:@"caption_link"]) {
            caption = [NSString stringWithFormat:@"<a href=\"%@\">%@</a>", link, caption];
        }
        
		OARequestParameter *captionParam = [[OARequestParameter alloc] initWithName:@"caption" value:caption];
		OARequestParameter *tweetParam = [[OARequestParameter alloc] initWithName:@"tweet" value:shouldTweet];
		OARequestParameter *tagsParam = [[OARequestParameter alloc] initWithName:@"tags" value:tags];
		OARequestParameter *slugParam = [[OARequestParameter alloc] initWithName:@"slug" value:slug];
		OARequestParameter *privateParam = [[OARequestParameter alloc] initWithName:@"state" value:private];
        OARequestParameter *sourceParam = [[OARequestParameter alloc] initWithName:@"source" value:source];
        OARequestParameter *linkParam = [[OARequestParameter alloc] initWithName:@"link" value:link];
		NSArray *params = [NSArray arrayWithObjects:typeParam, captionParam, tweetParam, tagsParam, slugParam, privateParam, linkParam, sourceParam, nil];
        [oRequest setParameters:params];
		[typeParam release];
